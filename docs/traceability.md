...
| REQ-F-002 | Interpret FuFire responses without guessing (bazi + wuxing) | AC-F-002a (paths in order, matched recorded), AC-F-002b (PROMPT_VARIABLE_SOURCE_MISSING), AC-F-002c (sanitized, no secret), AC-F-002d (deferred ops render-block), AC-F-002e (day-anchor unverified surfaced) | T4 | server/tests/fufire.responseInterpreter.test.ts (11/11; resolve animal/element/birth_year/dominant_element vs REAL samples; missing-field→PROMPT_VARIABLE_SOURCE_MISSING; 0,0-guard; day-pillar unverified surfaced; 3 mutations bite) | NO — built+sample-verified primitive, ZERO production importers (Gate C/D finding 2026-06-14): executeTestRun echoes the raw FuFire response UNMAPPED; interpreter not called on any live path; runner uses MockFuFireProvider in DEMO_LOCAL. Wiring deferred. | integration-fake (real captured samples, NOT a live call) | canvas | P2 | U-end | V-nofake, V-calc | S-builder | value-risk (interpretation not "verified truth"; day-pillar unverified surfaced; AND wired-in-prod=NO — built but not composed into a live path) |
| REQ-F-003 | Render prompt templates from safe prompt variables only (bazi + wuxing) | AC-F-003a (only mapped vars), AC-F-003b (missing var blocks render), AC-F-003c (no deterministic fortune claims) | T4 | server/tests/fufire.responseInterpreter.test.ts (render-block on missing var; only-mapped-vars; no fortune claims; single-pass render = no template injection) | NO — renderPromptTemplate is built+tested but not invoked on any live path (interpreter has zero production importers; Gate C/D finding 2026-06-14). Wiring deferred. | integration-fake (real samples) | canvas | P2 | U-end | V-nofake, V-calc | S-builder | value-risk (animal convention RESOLVED locale-driven; BUT wired-in-prod=NO — built but not composed into a live path) |
