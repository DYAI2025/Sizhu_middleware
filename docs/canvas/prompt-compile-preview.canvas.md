...
| Constraints | CAN-007 | Single-Express-Server; neue `/api`-Route in `apiGuard` klassifizieren (Default-Deny → min. `session`); kein `server/routes/`-Dir (Route in `server/index.ts` oder neues Dir). **Image-Text-Policy (hart, §1.2): das Bildmodell erzeugt keinen lesbaren Hanzi/Pinyin/Label/Datum-Text.** **Region: CN_SIMPLIFIED** (kein Traditional/JP/KR). Kohäsion: `BaziSymbolMapper` darf `fufireResponseInterpreter.ts` NICHT duplizieren — der Interpreter wird ERWEITERT (neue Felder), der Mapper hält die Hanzi-Tabelle (OQ-D, Phase-1-Planner). TDD + Guard-Test mit RED-on-revert (P1–P9). | EXPLICIT | SRC-002, SRC-003 |
