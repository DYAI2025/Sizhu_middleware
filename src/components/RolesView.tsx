...
CREATE POLICY read_own_profile ON app_users FOR SELECT USING (id = auth.uid());
  ...
  return (
    <div className="space-y-6 animate-fade-in text-da" id="roles-permissions-hud">
      
      {/* Title */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-nt pb-4">
        <div>
          <h1 className="text-xl font-bold text-da tracking-tight font-sans">Security & Account RBAC</h1>
          <p className="text-xs text-nt mt-1">Configure user roles simulation, manage database team roles, and view Postgres Row Level Security (RLS) scopes</p>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className={`px-3 py-1.5 font-mono text-xs font-bold rounded-sm border uppercase ${
              activeTab === 'matrix' 
                ? 'bg-b2 text-da border-da' 
                : 'bg-b1 text-nt border-nt hover:bg-b1'
            }`}
          >
            Permissions Matrix
          </button>
      ...
      <div className="bg-b2 text-da p-5 rounded-sm border border-da flex flex-col md:flex-row md:items-center justify-between gap-5 shadow-sm">
        <div className="space-y-1">
          <span className="text-[9px] bg-ac text-nt font-mono px-2 py-0.5 rounded-sm font-bold uppercase tracking-wider">Simulation Profile Selector</span>
          <h2 className="text-base font-bold font-mono uppercase tracking-wide mt-1">Override Current User Profile Context</h2>
          <p className="text-xs text-nt">Switching profiles overrides middleware token authentication permissions on the fly.</p>
        </div>
            ...
            <button
              ...
              className={`text-[10px] font-bold uppercase font-mono py-2 px-3.5 rounded-sm border transition cursor-pointer ${
                activeRoleName === r.role 
                  ? 'bg-ac text-nt border-ac shadow-sm' 
                  : 'bg-b2 border-nt text-nt hover:bg-b2'
              }`}
            >
              Simulate: {r.role}
            </button>
      ...
      {activeTab === 'matrix' ? (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 bg-b1 border border-nt rounded-sm p-4 space-y-4">
              <div className="border-b border-nt pb-2">
                <h3 className="text-xs font-bold uppercase tracking-widest text-da font-mono flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5 text-nt" />
                  ...
                  return (
                    <div 
                      ...
                      className={`p-3 rounded-sm border text-xs leading-relaxed space-y-1 transition ${
                        worksAsActive ? 'bg-b1/50 border-ac border-l-2 border-l-blue-500' : 'bg-b1 border-nt'
                      }`}
                    >
                      <div className="flex items-center justify-between font-mono">
                        <strong className="text-da font-bold text-[11px]">{r.role} profile</strong>
                        {worksAsActive ? (
                          <span className="text-[9px] bg-ac text-ac border border-ac font-mono font-bold px-1.5 py-0.2 rounded-sm uppercase tracking-wide">
                            ACTIVE
                          </span>
                        ) : null}
                      </div>
                      <p className="text-nt text-[11px] leading-snug">{r.description}</p>
                    </div>
                  );
                })}
              </div>

              <div className="bg-b1 border border-ac rounded-sm p-4 text-[11px] text-ac font-mono leading-normal">
            ...
            <div className="lg:col-span-8 bg-b1 border border-nt rounded-sm p-4 space-y-4 overflow-hidden">
              <div className="border-b border-nt pb-2">
                <h3 className="text-xs font-bold uppercase tracking-widest text-nt font-mono flex items-center gap-1.5">
                  <Settings className="w-3.5 h-3.5 text-nt" />
                  Permissions authentication Matrix
                </h3>
              </div>

              <div className="overflow-x-auto border border-nt rounded-sm">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-b1 border-b border-nt text-[10px] font-bold font-mono text-nt uppercase tracking-widest">
                      <th className="py-2.5 px-3">System Permissions Code</th>
                      {roles.map(r => (
                        <th key={r.role} className="py-2.5 px-3 text-center border-l border-nt">{r.role}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {permissions.map((p) => (
                      <tr key={p.id} className="hover:bg-b1/50 transition">
                        <td className="py-3 px-3">
                          <div className="font-bold text-da font-mono text-[11px] uppercase tracking-wide">{p.name}</div>
                          <div className="text-[10px] text-nt mt-0.5 leading-snug">{p.description}</div>
                        </td>
                        ...
                        {roles.map((r) => {
                          ...
                          return (
                            <td key={r.role} className="py-3 px-3 text-center border-l border-nt">
                              <button
                                disabled={isImmutable}
                                onClick={() => handleTogglePermission(r.role, p.id)}
                                className={`p-1 transition rounded-sm ${
                                  isImmutable ? 'opacity-80 cursor-not-allowed' : 'cursor-pointer hover:bg-b2'
                                }`}
                              >
                                {hasPerm ? (
                                  <Check className={`w-4 h-4 mx-auto ${
                                    isImmutable ? 'text-ac font-black' : 'text-da'
                                  }`} />
                                ) : (
                                  <span className="text-nt font-bold font-mono">&ndash;</span>
                                )}
                              </button>
                            </td>
                          );
          ...
          <div className="bg-b1 border border-nt rounded-sm p-4 space-y-4">
            <div className="border-b border-nt pb-2.5 flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-widest text-da font-mono flex items-center gap-1.5">
                <Users className="w-4 h-4 text-nt" />
                Team credentials directory (Supabase Replica)
              </h3>
              <span className="text-[10px] font-mono bg-b2 px-2 py-0.5 rounded border border-nt font-bold text-nt">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-4 bg-b1 border border-nt rounded-sm p-4 h-fit space-y-4">
                <div className="text-[10px] uppercase font-mono tracking-wider font-bold text-nt">Add Team credentials</div>
                <form onSubmit={handleAddUser} className="space-y-3 text-xs">
                  <div>
                    <label className="block text-[9px] font-black uppercase font-mono text-nt">Email Address (Supabase UID Link)</label>
                    <input
                      type="email"
                      required
                      ...
                      className="mt-1 w-full border border-nt bg-b1 rounded-sm p-2 outline-none"
                    />
                  </div>

                  <div>
                    <label className="block text-[9px] font-black uppercase font-mono text-nt">Authorization role</label>
                    <select
                      value={newUserRole}
                      onChange={(e) => setNewUserRole(e.target.value as any)}
                      className="mt-1 w-full border border-nt bg-b1 rounded-sm p-2 outline-none font-mono"
                    >
                  ...
                  <button
                    type="submit"
                    className="w-full bg-b2 hover:opacity-90 border border-da text-da p-2 font-mono font-bold text-xs rounded-sm uppercase flex items-center justify-center gap-1 cursor-pointer"
                  >
                    <Plus className="w-3.5 h-3.5 text-ac" /> Catalog User
                  </button>
                </form>

                <div className="bg-b1/10 p-3 rounded-sm border border-ac/50 text-[9px] text-ac leading-relaxed font-mono">
              ...
              {/* Simulated user rows */}
              <div className="lg:col-span-8 overflow-hidden rounded-sm border border-nt">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-b1 border-b border-nt text-[9.5px] font-bold font-mono text-nt uppercase tracking-widest">
                      <th className="py-2.5 px-3.5">User Email (OAuth Node)</th>
                      <th className="py-2.5 px-3 border-l border-nt">Authentication Role</th>
                      <th className="py-2.5 px-3 border-l border-nt">Created AT</th>
                      <th className="py-2.5 px-3 text-center border-l border-nt">Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-150">
                    {users.map((usr) => (
                      <tr key={usr.id} className="hover:bg-b2/10 transition leading-normal">
                        <td className="py-3 px-3.5 font-sans font-semibold text-da">
                          <div className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-ac"></span>
                            {usr.email}
                          </div>
                        </td>
                        <td className="py-2 px-3 border-l border-nt">
                          <select
                            value={usr.role}
                            onChange={(e) => handleUpdateUserRole(usr.id, e.target.value as any)}
                            className="bg-b1 border border-nt p-1 rounded-sm font-mono text-[10.5px] text-nt"
                          >
                        ...
                        <td className="py-3 px-3 border-l border-nt text-nt font-mono text-[10.5px]">
                          {new Date(usr.createdAt).toLocaleDateString()}
                        </td>
                        <td className="py-3 px-3 border-l border-nt text-center">
                          <button
                            onClick={() => handleDeleteUser(usr.id)}
                            className="text-nt hover:text-ac p-1 rounded hover:bg-b1 cursor-pointer"
                          >
        ...
        <div className="space-y-4 animate-fade-in font-mono text-xs">
          <div className="bg-b2 text-nt p-4 border border-da rounded-sm space-y-4">
            <div className="flex items-center justify-between border-b border-nt pb-3">
              <div className="flex items-center gap-3">
                <Terminal className="w-5 h-5 text-ac animate-pulse" />
                <div>
                  <h3 className="text-sm font-bold text-da leading-none">DATABASE RLS SCHEMATICS</h3>
                  <p className="text-[10px] text-nt mt-1">Execute these tables creation and triggers inside your Supabase SQL editor directly</p>
                </div>
              </div>

              <button
                onClick={handleCopySQL}
                className="bg-b2 hover:bg-b2 text-da text-[10px] font-bold border border-nt py-1.5 px-3 rounded flex items-center gap-1 transition-all cursor-pointer"
              >
                <Copy className="w-3.5 h-3.5 text-ac" />
                {copied ? 'COPIED!' : 'COPY DDL'}
              </button>
            </div>

            <div className="relative">
              <pre className="text-[10px] text-nt leading-relaxed overflow-x-auto p-4 bg-b2 border border-nt/80 rounded max-h-[460px] custom-scrollbar selection:bg-b2">
{`-- =========================================================================
...
$$ LANGUAGE plpgsql SECURITY DEFINER;`}
          ...
          <div className="p-3.5 bg-ac border border-ac text-ac rounded-sm font-sans flex items-start gap-2.5">
            <Info className="w-5 h-5 text-ac shrink-0 mt-0.5" />
            <div>
              <span className="font-bold block text-[11px] uppercase font-mono">Row Level Security Verification:</span>
              <p className="text-xs text-ac leading-relaxed mt-0.5">
