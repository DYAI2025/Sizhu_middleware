...
- **P9 — a cross-component safety guarantee is verified against the component that enforces it, never assumed.** When a capability's safety rests on a guard enforced ELSEWHERE (a downstream route, another layer/service), prove that guard actually fires *at that location* — cite its real call site (`file:line`) or a guard test — before claiming it. Existence of the guard function somewhere ≠ it being wired on the path you depend on. If it is not there: state the truth (do not record the guarantee), and **withhold / flag-off the dependent capability** (e.g. ship a money tool OFF by default) until the real guard lands — the missing guard is escalated, never laundered into "it's fine". This is the dependency-direction complement of P1 (P1 = is *my* symbol reachable in prod; P9 = is the *depended-on* guard real where I rely on it). (Origin: the sizhu-mcp-server documented "the server enforces `assertDispatchAllowed`" on the POD dispatch route as the payment gate; the security review found that guard had **zero** server-route callers — a fictional gate on a real-money path. Fixed by correcting the claim + withholding `pod_dispatch` behind `MCP_ENABLE_DISPATCH` until a real server-side approval gate exists.)

### Adopted from the sizhu-agent-safe-ops retro (2026-06-17)

- **P10 — the Canvas `Allowed change scope` must be a parseable `## ` section that names the process artifacts (operationalizes P5).** `plumbline-scope-check` (and the fail-closed PRIL Stop hook) read a markdown **`## Allowed change scope` section** with bullet globs — OR `docs/scope/<feature>.scope.json`. A Canvas **table row** is invisible to it (`EXIT_MISSING` → block). The section MUST list the process/verification artifact classes the run produces (`docs/canvas|prd|vision`, `docs/traceability.md`, `docs/plans/**`, `docs/context/**`, `docs/reality/**`, `metrics/**`, the test dirs), not just code paths — otherwise the Phase-0 docs commit itself trips the gate. (Origin: CAN-011 was a table row + code-only → two fail-closed Stop-hook blocks before the build could start.)
- **P11 — a feature branch stacked on an UNMERGED base branch false-positives the PRIL scope hook; land the base on `main` first.** The Stop hook computes the feature surface as `merge-base(HEAD, main)...HEAD`, so a feature cut off an unmerged feature branch drags the base feature's files into *your* scope surface → fail-closed block on files that aren't yours. Land the base feature on `main` (merge/PR) before stacking, so the merge-base is clean. **Never force-push `main`** to fix it — verify with `git merge-base --is-ancestor <base-head> origin/main`; a teammate's PR may already have merged the base (it did here). (Origin: sizhu-agent-safe-ops off `feat/lgq-live-guards`.)
- **P12 — verify a commit contains what its message claims, especially when a deletion is co-staged (extends P8a).** `git add -- <deleted-dir>` fails with "pathspec matches no files" and **aborts staging the co-listed paths**, so a commit can silently omit a change its message claims. Stage deletions in a separate `git add` (or `git add -A`) and run `git show --stat HEAD` to confirm the commit matches its message. **Correction to P8(b):** the `-F -` (stdin) heredoc works for `git commit` but **NOT `git merge`** (which treats `-` as a literal filename → "could not read file '-'"); write the merge message to a temp file and use `-F <tmpfile>`. (Origin: the T6 commit nearly shipped without its `package.json` change; a `git merge -F -` errored.)
