...
- **P4 — every behaviour-change ships a RED-on-revert mutation proof.** Until a mutation framework (Stryker) exists, manually revert the new guard to its pre-fix form, confirm the test goes RED, restore, and note it in the commit body. This is what turns a green-but-useless test into a trustworthy one. **Follow-up ticket: evaluate Stryker** (gives a measurable mutation score + the first `metrics/runs.jsonl` baseline).
